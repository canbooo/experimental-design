...
class TestDiscreteVariable:
    ...
    def test_value_of_with_mapper(self):
        ...
        assert var.value_of(1e-6) == 666
        assert var.value_of(1) == 42
        assert np.all(var.value_of(np.array([1e-6, 1])) == np.array(values))


def test_create_discrete_variables():
    variables = module_under_test.create_discrete_variables([[1, 2], [3, 4, 5], [9, 8]])
    probabilities = np.array([1e-6, 0.6, 1])
    expected = np.array([[1, 2, 2], [3, 4, 5], [9, 8, 8]])
...
def test_create_uniform_variables():
    variables = module_under_test.create_uniform_variables([1, 42, 665], [3, 52, 667])
